%------------------------------------------------------------------------%
%------ Gmsh to Matlab script: Import QUAD mesh to matlab----------------%
%------------------------------------------------------------------------%
...
% File name
file = 'chamfer.msh'; % Updated for quad mesh

% Read the entire file as text
mesh_data = fileread(file);

% Locate the $Nodes section
nodes_start = regexp(mesh_data, '\$Nodes', 'end') + 1;
nodes_end = regexp(mesh_data, '\$EndNodes') - 1;

% Debug: show positions of $Nodes and $EndNodes
fprintf('$Nodes starts at: %d\n', nodes_start);
fprintf('$EndNodes ends at: %d\n', nodes_end);

% Extract nodes
if isempty(nodes_start) || isempty(nodes_end) || nodes_start >= nodes_end
    error('Failed to locate valid $Nodes section in the .msh file.');
end
...
if isempty(nodes_block)
    error('Could not locate $Nodes section in the .msh file.');
end
...
for i = 2:num_elements+1
    data = sscanf(elements_block{i}, '%d %d %d %d %d %d %d %d'); % <- Notice it's 8 now
    if numel(data) < 8
        warning('Element data on line %d is incomplete. Skipping.', i);
        continue;
    end
    if data(2) == 3 % Element type 3 = 4-node quadrilateral
        quad_elements = [quad_elements; data(5:8)']; % Grabbing the last 4 items
    end
end
...
title('GMsh to MATLAB import (Quad Mesh)', 'fontsize', 14)
set(gcf, 'color', 'white');
